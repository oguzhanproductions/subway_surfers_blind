...
from subway_blind.translation import set_language, translate_text
from subway_blind.version import APP_VERSION, APP_WINDOW_TITLE

def main() -> None:
    settings = load_settings()
    settings["language"] = set_language(settings.get("language"))
    pygame.init()
    settings[_sx(1)] = initialize_mixer_output(settings.get(_sx(1))) or _sx(2)
    pygame.display.set_caption(translate_text(_sx(3).format(APP_WINDOW_TITLE, APP_VERSION)))
