...
class AudioTests(unittest.TestCase):
    ...
    def test_resolve_playback_channel_prefers_free_polyphony_slot(self):
        audio = Audio.__new__(Audio)
        audio.channels = {
            "player_pickup__0": type("Channel", (), {"get_busy": lambda self: True})(),
            "player_pickup__1": type("Channel", (), {"get_busy": lambda self: False})(),
        }
        audio._channel_polyphony_index = {"player_pickup": 0}
        audio.hrtf = type("Hrtf", (), {"is_channel_playing": lambda self, _name: False})()

        selected = audio._resolve_playback_channel("player_pickup", loop=False)

        self.assertEqual(selected, "player_pickup__1")
...
class GameTests(unittest.TestCase):
    ...
    def test_keyboard_binding_capture_updates_menu_confirm(self):
        ...
        game._handle_keyboard_event(pygame.event.Event(pygame.KEYDOWN, {"key": pygame.K_f, "mod": 0}))
        game._update_keyboard_binding_hold(3.0)
    ...
    def test_keyboard_binding_capture_supports_modifier_combo(self):
        game, speaker, _ = self.make_game()
        game._build_keyboard_bindings_menu()
        game.active_menu = game.keyboard_bindings_menu

        game._begin_binding_capture("keyboard", "menu_confirm")
        game._handle_keyboard_event(pygame.event.Event(pygame.KEYDOWN, {"key": pygame.K_LSHIFT, "mod": pygame.KMOD_LSHIFT}))
        game._handle_keyboard_event(pygame.event.Event(pygame.KEYDOWN, {"key": pygame.K_s, "mod": pygame.KMOD_LSHIFT}))
        game._update_keyboard_binding_hold(3.0)
    ...
    def test_keyboard_binding_capture_fails_when_key_released_early(self):
        ...
        game._begin_binding_capture("keyboard", "menu_confirm")
        game._handle_keyboard_event(pygame.event.Event(pygame.KEYDOWN, {"key": pygame.K_f, "mod": 0}))
        game._handle_keyboard_event(pygame.event.Event(pygame.KEYUP, {"key": pygame.K_f, "mod": 0}))

        self.assertEqual(game.controls.keyboard_binding_for_action("menu_confirm"), pygame.K_RETURN)
        self.assertIn(("Binding failed. Keep the selected keys pressed for the full 3 seconds.", True), speaker.messages)
